...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    for(int i=shape.size - 1;i >=0;i--){
      ...
      size_t my_off = gid % cur_size / pre_size;
      idx += my_off*strides.data[i];
      pre_size = cur_size;
    }
    out[gid] = a[idx];
  }
...
__global__ void ScalarMulKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size){
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size){
    out[gid] = a[gid] * val;
  }
...
__global__ void ScalarDivKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size){
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size){
    out[gid] = a[gid] / val;
  }
...
__global__ void ScalarEqKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size){
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size){
    out[gid] = a[gid] == val;
  }
...
__global__ void ScalarGeKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size){
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size){
    out[gid] = a[gid] >= val;
  }
